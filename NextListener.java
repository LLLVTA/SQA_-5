...
public class NextListener implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e){
        ...
        if(brnstr == "NextDate"){
            ...
            //检查是否为空
            boolean flag = true;
            if(ly.length() <= 0)  flag = false;
            else if(lm.length() <= 0)  flag = false;
            else if(ld.length() <= 0)  flag = false;

            //检查非法字符
            for(int i = 0; i < ly.length(); i++){
                if(i == 0 && ly.charAt(0) == '-')  continue;
...
//            System.out.println(flag);
            ...
            if(flag == false)  JOptionPane.showMessageDialog(null,"参数输入非法");
                ...
                if(ily < 1 || ilm < 1 || ild < 1)  JOptionPane.showMessageDialog(null, "参数输入超限");
                else{
                    ...
                    if(c3 > date[ru][c2])  NextDate.xoutput.setText("不可能");
                    else{
                        c3++;
                        if(c3 > date[ru][c2]){
                            c3 = 1;
